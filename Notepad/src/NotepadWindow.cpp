...
bool NotepadWindow::Create(NotepadWindow** pNotepadWindow, HINSTANCE moduleInstance)
{
	...
	hRes = NativeHWNDHost::Create((LPCWSTR)L"Leet's DUI Notepad", NULL, NULL, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, 0, WS_OVERLAPPEDWINDOW, NULL, &pNpWindow->_pNativeWindowHost);
	DUIAssert(hRes == S_OK, "Failed to create native window host.");

	// Create a main window element
	DWORD dwDeferKey;
	hRes = HWNDElement::Create(pNpWindow->_pNativeWindowHost->GetHWND(), true, 0, NULL, &dwDeferKey, &pWindowElement);
	...
	pParser->SetXMLFromResource((LPCWSTR)MAKEINTRESOURCEW(IDR_DUIPADUI), moduleInstance, moduleInstance);

	// Create the main UI element
	pParser->CreateElement(reinterpret_cast<LPCWSTR>(L"duipad"), pWindowElement, NULL, NULL, &pNpWindow->_pWindowElement);
	...
	// Initialize members
	pNpWindow->_pParseButton = (Button*)pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"parsebutton")));
	pNpWindow->_pAutoUpdateCheck = (CCCheckBox*)pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"autoupdatecheck")));
	pNpWindow->_pStatus = pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"status")));
	pNpWindow->_pErrorBox = pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"errorbox")));
	pNpWindow->_pContainer = pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"container")));
	pNpWindow->_pEdit = pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"edit")));
	pNpWindow->_pMarkupBox = pNpWindow->_pWindowElement->FindDescendent(StrToID(reinterpret_cast<LPCWSTR>(L"markupbox")));
	...
	pNpWindow->_pEdit->SetContentString((LPCWSTR)pResBuffer);

	pNpWindow->_pErrorBox->SetVisible(false);
	pNpWindow->_pErrorBox->SetLayoutPos(-3); // none

	pNpWindow->_pWindowElement->EndDefer(dwDeferKey);
		...
		case ID_FILE_OPEN:
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					_pEdit->SetContentString((LPCWSTR)text);
					free(text);
...
void NotepadWindow::OnEvent(Element* elem, Event* pEvent)
{
	...
	{
		if (pEvent->peTarget == _pParseButton)
		{
			Refresh(true);
		}
	}
...
void NotepadWindow::Refresh(bool showError)
{
	...
	if (lstrlenW(pTextW) > 0)
	{
		...
		hr = pParser->SetXML(reinterpret_cast<LPCWSTR>(pTextW), _modInstance, NULL);
		if (SUCCEEDED(hr))
		{
			Element* pe = nullptr;
	...
	if (_szParseError[0])
	{
		_pStatus->SetContentString(reinterpret_cast<LPCWSTR>(L"Failed"));
		_pStatus->SetForegroundColor(RGB(128, 0, 0));
		if (showError)
		{
			_pErrorBox->SetContentString(reinterpret_cast<LPCWSTR>(_szParseError));
	...
	{
		_pStatus->SetContentString(reinterpret_cast<LPCWSTR>(L"Successful"));
